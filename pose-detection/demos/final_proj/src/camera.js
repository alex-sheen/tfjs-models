 ...
 */
 ...
 export class Camera {
   ...
   drawCtx() {
     ...
     this.ctx.stroke();
     this.ctx.fill();
     this.ctx.save(); //default state
 
     this.time += 1;
     if (this.speed_up > 20) {
       this.speed_up -= 90
       this.speed_up = this.speed_up / 110 * 2
   ...
   drawResults(poses) {
     ...
     if (poses.length == 2) {
       this.distance_hands = Math.min(500, this.dist(poses[0].keypoints[10].x, poses[0].keypoints[10].y, poses[1].keypoints[10].x, poses[1].keypoints[10].y));
     }
   }
   ...
   drawKeypoints(keypoints, index) {
     ...
     for (let x = 10; x < width; x += 15) {
       for (let y = 10; y < height; y += 15) {
         ...
         let distance_right = this.dist(keypoints[9].x, keypoints[9].y, x, y);
         let distance_left = this.dist(keypoints[10].x, keypoints[10].y, x, y);
         let distance_left_right = this.dist(keypoints[9].x, keypoints[9].y, keypoints[10].x, keypoints[10].y);
         let distance_left_head = this.dist(keypoints[0].x, keypoints[0].y, keypoints[10].x, keypoints[10].y);
         let distance_right_head = this.dist(keypoints[0].x, keypoints[0].y, keypoints[9].x, keypoints[9].y);
         ...
         let distance_elbows = this.dist(keypoints[8].x, keypoints[8].y, keypoints[7].x, keypoints[7].y);
         ...
         //console.log(distance_elbows);
         let rect_size = 15; //10 + (distance_elbows) / 120; 
 
 
         let color_scale = 1.5;
         if (distance_left_right > 255 * color_scale)  {
           distance_left_right = 255 * color_scale;
         }
         ...
         this.ctx.beginPath();
         let rgb = hslToRgb(distance_left_right / color_scale / 255 + distance / color_scale / 255, distance_right_head / color_scale / 255 / 2 + distance_left_head / color_scale / 255 / 2, 150 / 255);
        ...
        // this.ctx.beginPath();
         ...
         //let a = Math.pow((Math.pow(rect_size,2) / 2), 1/2); 
         //let lamda = ((x/rect_size * y/rect_size) % n);
        // let rect_scale = rect_size * Math.sqrt((n)/2); 
        ...
        //console.log(n);
         ...
         // let n_prime = noise.perlin3(x * 0.005, y * 0.005, Math.abs(this.time) * 0.02);
         // n_prime = Math.abs(n_prime);
         // let rect_scale = rect_size * Math.atan(n_prime); 
         // let x1 = Math.sin(Math.PI * 3 * n_prime) * rect_scale;
         // let y1 = Math.cos(Math.PI * 3 * n_prime) * rect_scale;
         
         let base = 0
         if (this.speed_up > 20) {
           base = (this.speed_up - 90) / 110 * 3
         }
 
 
         rect_size = 30;
         let rect_scale = rect_size * Math.atan(n) + base;
         let increase = distance_right_head / color_scale / 255 * 3;
 
         
         if (rect_scale > 13 + increase){
           rect_scale = 13 + increase;
         }
         let x1 = Math.sin(Math.PI * 2 * (base + n + this.distance_hands / 500 * 2)) * rect_scale;
         let y1 = Math.cos(Math.PI * 2 * n) * rect_scale;
 
         this.ctx.moveTo(x,y);
         this.ctx.lineTo(x+x1,y+y1);
         this.ctx.lineTo(x+x1-y1, y+y1+x1);
         this.ctx.lineTo(x-y1, y+x1);
         this.ctx.closePath();
 
         // this.ctx.moveTo(x,y+lamda);
         // this.ctx.lineTo(x+lamda,y+rect_size);
         // this.ctx.lineTo(x+rect_size, y+rect_size-lamda);
         // this.ctx.lineTo(x+rect_size-lamda,y);
         // this.ctx.closePath();
 
         //this.ctx.rect(x,y,rect_size,rect_size);
         this.ctx.fill();
       }
     }
